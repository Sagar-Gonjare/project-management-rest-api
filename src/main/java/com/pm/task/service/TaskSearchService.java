...
public class TaskSearchService {
    ...
    public TaskSearchService(TaskSearchRepository repo, UserRepository userRepo) {
        this.repo = repo;
        this.userRepo = userRepo;
    }

    @Transactional(readOnly = true)
    public Page<TaskResponse> searchMine(String q, TaskStatus status, TaskPriority priority, Pageable pageable) {
        Users user = currentUserOrThrow();

        return repo.searchMine(user.getId(), q, status, priority, pageable)
                .map(this::toResponse);
    }
